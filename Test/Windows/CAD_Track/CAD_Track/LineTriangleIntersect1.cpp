...
static int  CalcThreeCutSurface(threespace TF[3], float CutSurface[4], float* out_CutSurfaces, float* sign) {
    ...
    memcpy(TF2, TF, sizeof(threespace) * 3);
    ...
    ret = CalcCutSurface(TF1, out_CutSurfaces + 0);
    ret = CalcCutSurface(TF2, out_CutSurfaces + 4);
    ret = CalcCutSurface(TF3, out_CutSurfaces + 8);
    ...
    float a = InTriangle.real_x*out_CutSurfaces[0] + InTriangle.real_y * out_CutSurfaces[1] + InTriangle.real_z * out_CutSurfaces[2] + out_CutSurfaces[3];
    float b = InTriangle.real_x*out_CutSurfaces[4] + InTriangle.real_y * out_CutSurfaces[5] + InTriangle.real_z * out_CutSurfaces[6] + out_CutSurfaces[7];
    float c = InTriangle.real_x*out_CutSurfaces[8] + InTriangle.real_y * out_CutSurfaces[9] + InTriangle.real_z * out_CutSurfaces[10] + out_CutSurfaces[11];
...
int CalcFive(void* ptr, CalcFives* Calcs)
{
    ...
    //获取每一个三角片面上的垂直于三角片面并且过一条边的直线方程和sign值
    for (int ss = 0; ss < gli->NumFaceList; ss++)
    {
        threespace TF[3] = { 0.0f };
        //Point3f FacePnt1,FacePnt2, FacePnt3;



        TF[0].real_x = gli->MyFaceListPtr[ss].facePoint[0].v[0];
        TF[0].real_y = gli->MyFaceListPtr[ss].facePoint[0].v[1];
        TF[0].real_z = gli->MyFaceListPtr[ss].facePoint[0].v[2];

        TF[1].real_x = gli->MyFaceListPtr[ss].facePoint[1].v[0];
        TF[1].real_y = gli->MyFaceListPtr[ss].facePoint[1].v[1];
        TF[1].real_z = gli->MyFaceListPtr[ss].facePoint[1].v[2];

        TF[2].real_x = gli->MyFaceListPtr[ss].facePoint[2].v[0];
        TF[2].real_y = gli->MyFaceListPtr[ss].facePoint[2].v[1];
        TF[2].real_z = gli->MyFaceListPtr[ss].facePoint[2].v[2];


        float CutSurfaces[4] = { 0.0f };

        ret = CalcCutSurface(TF, CutSurfaces);
        //CutSurfaceAll.push_back(CutSurface);

        ret = CalcThreeCutSurface(TF, CutSurfaces, Calcs[ss].CutSurfaces, Calcs[ss].sign);
        memcpy(Calcs[ss].CutSurfaces + 12, CutSurfaces, sizeof(float) * 4);
    }
    return ret;
}
...
bool ValidPointf_byThree(threespace LinePoint1, threespace LinePoint2, CalcFives TrianglePoint, threespace* result)
{
    ...
    */
    ...
    tempU = TrianglePoint.CutSurfaces[12] * LinePoint1.real_x + TrianglePoint.CutSurfaces[13] * LinePoint1.real_y + TrianglePoint.CutSurfaces[14] * LinePoint1.real_z + TrianglePoint.CutSurfaces[15];
    tempD = TrianglePoint.CutSurfaces[12] * LineV.real_x + TrianglePoint.CutSurfaces[13] * LineV.real_y + TrianglePoint.CutSurfaces[14] * LineV.real_z;  //判断平面法向量和直线的方向向量是否垂直
    ...
    float rd1 = tempU;
    float rd2 = TrianglePoint.CutSurfaces[12] * LinePoint2.real_x + TrianglePoint.CutSurfaces[13] * LinePoint2.real_y + TrianglePoint.CutSurfaces[14] * LinePoint2.real_z + TrianglePoint.CutSurfaces[15];
    ...
    float a = CrossPoint.real_x * TrianglePoint.CutSurfaces[0] + CrossPoint.real_y * TrianglePoint.CutSurfaces[1] + CrossPoint.real_z * TrianglePoint.CutSurfaces[2] + TrianglePoint.CutSurfaces[3];
    float b = CrossPoint.real_x * TrianglePoint.CutSurfaces[4] + CrossPoint.real_y *TrianglePoint.CutSurfaces[5] + CrossPoint.real_z * TrianglePoint.CutSurfaces[6] + TrianglePoint.CutSurfaces[7];
    float c = CrossPoint.real_x * TrianglePoint.CutSurfaces[8] + CrossPoint.real_y *TrianglePoint.CutSurfaces[9] + CrossPoint.real_z * TrianglePoint.CutSurfaces[10] + TrianglePoint.CutSurfaces[11];

    if (a*TrianglePoint.sign[0] < 0 || b*TrianglePoint.sign[1] < 0 || c*TrianglePoint.sign[2] < 0) {
        return false;
    }
